-- Panel pages: admin setup, leave card, monthly attendance batch

-- ---------------------------------------------------------------------------
-- Promote admin by email (run once; safe to re-run)
-- ---------------------------------------------------------------------------
...
-- Default special message keys (no-op if exist)
...
begin
  ...
  select jsonb_build_object(
    ...
    'TeacherSub', t.teacher_sub,
    'MobileNo', t.mobile_no
  ) into v_teacher
  from public.teachers t
  where t.index_no = p_teacher_index;
  ...
  where l.teacher_index = p_teacher_index;

  for r in
    select leave_start, leave_end
    from public.leaves
    where teacher_index = p_teacher_index
  loop
    v_days := (r.leave_end - r.leave_start) + 1;
    if extract(year from r.leave_start) = extract(year from current_date) then
      v_yearly := v_yearly + v_days;
    end if;
  ...
  return jsonb_build_object(
    ...
    'teacher', v_teacher,
    'leaves', v_leaves,
    'monthlyLeaveDays', v_monthly,
    'yearlyLeaveDays', v_yearly
  );
end;
$$;
...
-- ---------------------------------------------------------------------------
-- Monthly attendance upsert (office / admin)
-- ---------------------------------------------------------------------------
create or replace function public.upsert_monthly_attendance(
  p_year integer,
  p_month integer,
  p_class text,
  p_rows jsonb
...
begin
  ...
  if not public.is_office_or_admin() then
    raise exception 'Insufficient permissions';
  end if;
  ...
  loop
    ...
    where class = p_class and roll = v_roll
    limit 1;
  ...
  return jsonb_build_object(
    'status', 'success',
    'message', v_count || ' জন শিক্ষার্থীর হাজিরা সংরক্ষিত হয়েছে।'
  );
end;
$$;
...
-- ---------------------------------------------------------------------------
-- Office: submit leave for any teacher
-- ---------------------------------------------------------------------------
create or replace function public.office_submit_leave(
...
-- Allow duty roster writes for admin (reassign)
create policy "duty_roster_daily_admin_write"
  on public.duty_roster_daily for all to authenticated
  using (public.is_admin())
  with check (public.is_admin());

create policy "temporary_duties_admin_write"
  on public.temporary_duties for all to authenticated
  using (public.is_admin())
  with check (public.is_admin());
