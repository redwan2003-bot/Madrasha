-- Safe to run after main schema. Copy of 20250522100004_leave_and_batch.sql
...
begin
  ...
  loop
    ...
    insert into public.reports (
      ...
      coalesce(r->>'Class', ''),
      coalesce(r->>'TeacherName', ''),
      coalesce((r->>'NumberOfAttend')::int, 0),
      coalesce(r->>'monitorReport', 'N/A'),
      v_monitor_index,
      v_monitor_index
    );
    v_count := v_count + 1;
  end loop;
  return jsonb_build_object('status', 'success', 'message', v_count || 'টি রিপোর্ট সফলভাবে জমা হয়েছে।');
end;
$$;
